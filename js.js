...
const clear = () => {
    ctx.fillStyle = 'rgba(233, 115, 115,.2)';
...
const posText = (txt) => {
    ...
    for(let i=0; i<m.data.length; i+=12) {
...
class Ball{
    constructor(x, y){
        ...
        this.boom = false
        this.s = {x:0,y:0}
        this.c = 'rgba(255,255,255,.2)'
    }
    draw() {
        if(this.show){
            ctx.beginPath();
            ctx.fillStyle = this.c
            ctx.arc(this.x+=this.s.x, this.y+=this.s.y, 1, 0, 2*Math.PI);
            ctx.fill();        
            ctx.closePath();
        }
    }    
...
class Bomb{
    ...
    draw() {
        ctx.beginPath();
        ctx.fillStyle = sort ? 'rgba(255,0,0,0.2)' : 'rgba(255,250,150,0.4)';
...
const init = () => {
    ...
    interval = setInterval(()=>{
        if(index+5<balls.length-1){
            for(let a=0;a<5;a++){
                for(let i=0;i<5;i++)bombs.push(new Bomb(balls[index].x,balls[index].y))
            ...
            for(let i=0;i<balls.length;i++){
                if(!balls[i].bomb){
                    balls[i].s.x =random(2,-2)
                    balls[i].s.y =random(3,-3)
                    balls[i].bomb = true
                    balls[i].c = "rgba(255,0,0,.3)"
                }
            }
        }      
    }, 10)  
